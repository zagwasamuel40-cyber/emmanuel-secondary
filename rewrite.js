...
let content = fs.readFileSync('src/pages/StudentPortalManager.tsx', 'utf8');

const imports = `import { useAnnouncements, Announcement } from "../data/announcementsData";
...
content = content.replace(/import \{ useAnnouncements, Announcement \} from "\.\.\/data\/announcementsData";\nimport \{ useResultsRelease, isResultReleased \} from "\.\.\/data\/resultsReleaseData";/, imports);

const stateCode = `  const [announcements, setAnnouncements] = useAnnouncements();
  const [releaseMap, updateRelease] = useResultsRelease();
  const [activeTab, setActiveTab] = useState("features");
  const [admissionApps, setAdmissionApps] = useAdmissionApps();`;

content = content.replace(/  const \[announcements, setAnnouncements\] = useAnnouncements\(\);\n  const \[releaseMap, updateRelease\] = useResultsRelease\(\);/, stateCode);

// Add Navigation Tabs before <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
const tabsUI = `      <div className="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap gap-2 mb-6">
        <button 
...
content = content.replace(/      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">/, tabsUI + '\n      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">');
...
content = content.replace(/      \{\/\* MODAL: VIEW ANNOUNCEMENT DETAILS \*\/\}/, adminSection + '\n      {/* MODAL: VIEW ANNOUNCEMENT DETAILS */}');

fs.writeFileSync('src/pages/StudentPortalManager.tsx', content);
